...
S3_DIR_NAME = "lawtalk/solved_cases"


def collect_and_save_single_solved_page(session_cookie: Any, category_name: str, category_id: str, offset: int, file_number: int,
                                       simple_result: bool = False, storage_type: bool = True, 
                                       only_new: bool = False) -> Dict[str, Any]:
    """
    단일 offset(페이지)의 로톡 해결된 사례를 수집하여 저장합니다.
    ...
    Returns:
        dict: 수집 및 저장 결과
    """
    logger.info(f"Processing category {category_name}, offset {offset}")
    
    try:
        # 단일 offset에 대해 데이터 수집
        response = request_solved_cases(session_cookie, category_id, offset)
        
        if not response.get('success'):
            logger.error(f"Failed to request solved cases data for {category_name} at offset {offset}")
            return {'success': False, 'error': 'Request failed', 'category': category_name, 'offset': offset}
        
        # 데이터 처리
        solved_data = process_solved_cases(response, simple_result, only_new, storage_type, category_name)
        
        if not solved_data.get('success'):
            logger.error(f"Failed to process solved cases data for {category_name} at offset {offset}")
            return {'success': False, 'error': 'Processing failed', 'category': category_name, 'offset': offset}
        
        # early_stop_triggered가 True면 크롤링 중단
        if solved_data.get('early_stop_triggered'):
        ...
        # 단일 offset에 대한 결과 구성
        posts = solved_data.get('data', {}).get('posts', [])
        ...
        # 카테고리별 디렉토리로 저장
        s3_dir_name = f"solved_cases/{category_name}"
        ...
        if save_result.get('success'):
            logger.info(f"Successfully saved {len(posts)} posts from {category_name} offset {offset}")
            return {
                'success': True,
                'category': category_name,
                'offset': offset,
...
def request_solved_cases(session_cookie: Any, category_id: str, offset: int = 0) -> Dict[str, Any]:
    """
    로톡 해결된 사례 목록을 API를 통해 가져옵니다.
    ...
    """
    url = config.SOLVED_CASES_URL
    
    # config에서 기본 파라미터 가져와서 동적으로 설정
    params = config.SOLVED_CASES_PAYLOAD_KEYS.copy()
    params.update({
    ...
    logger.debug(f"Requesting solved cases with category: {category_id}, offset: {offset}")
    ...
    try:
        ...
        logger.debug(f"Successfully fetched {posts_count} solved cases from category {category_id}, offset {offset}")
...
def process_solved_cases(response_data: Dict[str, Any], simple_result: bool = False, 
                        only_new: bool = False, storage_type: bool = True, 
                        category_name: str = "") -> Dict[str, Any]:
    """
    로톡 해결된 사례 API 응답 데이터를 simpleResult 인자에 따라 정리합니다.
    
    Args:
        response_data: request_solved_cases에서 받은 API 응답 데이터
    ...
    logger.debug(f"Processing {len(posts)} posts with simple_result={simple_result}, only_new={only_new}")
    ...
    logger.debug(f"Successfully processed {len(processed_posts)} posts")
    
    return result


def get_all_categories_solved_cases(session_cookie: Any, start_offset: int = 0, end_offset: int = None,
                                  simple_result: bool = False, storage_type: bool = True, 
                                  only_new: bool = False) -> Dict[str, Any]:
    """
    모든 카테고리의 해결된 사례를 수집합니다.
    ...
    logger.info(f"Starting solved cases collection for all categories: offset {start_offset} to {end_offset}, simple_result={simple_result}, only_new={only_new}")
    ...
    for category_name, category_id in config.CATEGORY_IDS.items():
        ...
        # 각 카테고리별로 페이지네이션 수행
        category_results = collect_category_solved_cases_with_pagination(
        ...
        if not storage_type and category_results.get('success'):
            bucket = config.AWS_S3_BUCKET
            s3_dir_name = f"{S3_DIR_NAME}/{category_name}"
    ...
    logger.info(f"Solved cases collection completed: {total_posts} total posts in {total_files} files across {len(config.CATEGORY_IDS)} categories")
    
    return final_result


def collect_category_solved_cases_with_pagination(session_cookie: Any, category_name: str, category_id: str,
                                                 start_offset: int = 0, end_offset: int = None,
                                                 simple_result: bool = False, storage_type: bool = True, 
                                                 only_new: bool = False) -> Dict[str, Any]:
    """
    특정 카테고리의 해결된 사례를 페이지별로 수집하고 저장합니다.
    ...
    logger.info(f"Starting {category_name} collection: offset {start_offset} to {end_offset}")
    
    while current_offset < end_offset:
        result = collect_and_save_single_solved_page(
            session_cookie=session_cookie,
            category_name=category_name,
            category_id=category_id,
    ...
    logger.info(f"{category_name} collection completed: {total_posts} total posts in {total_files} files")
